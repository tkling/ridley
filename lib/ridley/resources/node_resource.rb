module Ridley
  class NodeResource < Ridley::Resource
    ...
    # @param [String] host
    # @option [Boolean] sudo
    #   defaults to false, kick off chef run with sudo rights
    #
    # @return [HostConnector::Response]
    def chef_run(host, sudo = false)
      host_commander.chef_client(host, ssh: ssh, winrm: winrm, sudo: sudo)
    end
    ...
    #
    # @return [HostConnector::Response]
    def put_secret(host, sudo = false)
      host_commander.put_secret(host, encrypted_data_bag_secret, ssh: ssh, winrm: winrm, sudo: sudo)
    end
    ...
    #
    # @return [HostConnector::Response]
    def ruby_script(host, command_lines, sudo = false)
      host_commander.ruby_script(host, command_lines, ssh: ssh, winrm: winrm, sudo: sudo)
    end
    ...
    def run(host, command, sudo = false)
      host_commander.run(host, command, ssh: ssh, winrm: winrm, sudo: sudo)
    end
